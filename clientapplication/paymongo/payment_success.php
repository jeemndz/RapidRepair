    ...
    <main class="flex-1 overflow-y-auto flex flex-col">
        <header class="sticky top-0 z-40 w-full border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-8 h-16">
            ...
            <div class="flex items-center gap-4">
                <div class="relative">
                    <button id="notificationBtn" type="button" class="relative p-2 text-slate-500 hover:text-primary transition-all rounded-lg hover:bg-slate-100">
                        <span class="material-symbols-outlined">notifications</span>
                        <span id="notificationBadge" class="hidden absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 rounded-full bg-red-500 text-white text-[10px] font-black flex items-center justify-center">0</span>
                    </button>

                    <div id="notificationDropdown" class="hidden absolute right-0 top-full mt-3 w-96 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                        <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                            ...
                            <button id="clearNotificationsBtn" type="button" class="text-xs font-bold text-primary hover:underline">Clear</button>
                        </div>
                ...
                <button class="p-2 text-slate-500 hover:text-primary transition-all rounded-lg hover:bg-slate-100">
...
<div id="toastContainer" class="fixed top-5 right-5 z-[9999] space-y-3 w-[360px] max-w-[calc(100vw-2rem)]"></div>
...
class NotificationManager {
    ...
    showToast(item) {
        ...
        toast.className = `pointer-events-auto border ${tone.toast} rounded-xl shadow-lg p-4 transform transition-all duration-300 translate-x-6 opacity-0`;
        toast.innerHTML = `
            <div class="flex items-start gap-3">
                <div class="w-9 h-9 rounded-lg ${tone.icon} flex items-center justify-center flex-shrink-0">
                    <span class="material-symbols-outlined text-xl">${this.getIcon(item.type)}</span>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-black ${tone.title}">${this.escapeHtml(item.title)}</p>
                    ${item.message ? `<p class="text-xs ${tone.message} mt-0.5">${this.escapeHtml(item.message)}</p>` : ''}
                    <p class="text-[10px] text-slate-400 mt-1">${this.formatTime(item.time)}</p>
                </div>
                <button type="button" class="toast-close text-slate-400 hover:text-slate-700">
    ...
    renderNotifications() {
        if (!this.notificationList || !this.notificationBadge) {
            return;
        }
        ...
        this.notificationBadge.textContent = this.items.length > 9 ? '9+' : String(this.items.length);
        this.notificationBadge.classList.remove('hidden');

        this.notificationList.innerHTML = this.items.map(item => {
            const tone = this.getTone(item.type);

            return `
                <div class="px-4 py-3 border-b border-slate-100 hover:bg-slate-50">
                    <div class="flex items-start gap-3">
                        <div class="w-8 h-8 rounded-lg ${tone.icon} flex items-center justify-center flex-shrink-0">
                            <span class="material-symbols-outlined text-lg">${this.getIcon(item.type)}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-bold text-slate-900">${this.escapeHtml(item.title)}</p>
                            ${item.message ? `<p class="text-xs text-slate-500 mt-0.5">${this.escapeHtml(item.message)}</p>` : ''}
                            <p class="text-[10px] text-slate-400 mt-1">${this.formatTime(item.time)}</p>
                        </div>
                    </div>
                </div>
...
class RoleManager {
    ...
    showSuccess(message) {
        window.appNotifications?.success(message);
    }

    showError(message) {
        window.appNotifications?.error(message);
    }
...
class ServiceManager {
    ...
    async handleFormSubmit(e) {
        ...
        try {
            ...
            if (result.success) {
                let successMessage = result.message || 'Service saved successfully';

                if (typeof result.sub_services_added !== 'undefined' && Number(result.sub_services_added) > 0) {
                    successMessage += ` (${result.sub_services_added} sub-service${Number(result.sub_services_added) === 1 ? '' : 's'} added)`;
                }
...
document.addEventListener('DOMContentLoaded', () => {
    window.appNotifications = new NotificationManager();
