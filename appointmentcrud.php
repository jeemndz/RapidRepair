...
// Get data from POST body or GET query (try POST first, then GET)
$input = [];
...
// If empty, try GET query parameters
if (empty($input) && $_SERVER['REQUEST_METHOD'] === 'GET') {
  $input = $_GET;
}

// For create action, also check if action is specified in query string
$action = $_GET['action'] ?? $_POST['action'] ?? 'create';

// Validate and sanitize inputs
// Default tenantID to 1 if missing or invalid (matches frontend logic)
...
// Handle service_ids - could be JSON string from query param
$service_ids_raw = isset($input['service_ids']) ? $input['service_ids'] : [];
if (is_string($service_ids_raw)) {
  // Try to parse as JSON first
  $parsed = json_decode($service_ids_raw, true);
  $service_ids = is_array($parsed) ? $parsed : explode(',', $service_ids_raw);
} else {
